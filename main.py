...
BALL_SPEED = [random.randint(5, 10), random.randint(5, 10)]
...
PLAYER2.center = (550, WINDOW.get_height()/2)

BALL = pygame.Rect(0, 0, 15, 15)
BALL.center = (WINDOW.get_width()/2, random.randint(30, 370))
...
def p1_movement_input(window, player1):
    ...
    if keypressed[pygame.K_w] and not keypressed[pygame.K_s]:
        ...
        P1_VELOCITY = [0, PLAYER_SPEED] if player1.bottom < window.get_height() else [0, 0]        
    else:
        P1_VELOCITY = [0, 0]
    return P1_VELOCITY

def p2_movement_input(window, player2):
    global PLAYER_SPEED, P1_VELOCITY
    keypressed = pygame.key.get_pressed()
    if keypressed[pygame.K_UP] and not keypressed[pygame.K_DOWN]:
        ...
        P2_VELOCITY = [0, PLAYER_SPEED] if player2.bottom < window.get_height() else [0, 0]        
    else:
        P2_VELOCITY = [0, 0]
    return P2_VELOCITY
...
while True:
    ...
    # Players
    P1_VELOCITY = p1_movement_input(WINDOW, PLAYER1)
    P2_VELOCITY = p2_movement_input(WINDOW, PLAYER2)
